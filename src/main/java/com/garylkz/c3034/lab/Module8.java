...
public class Module8 {
    public static void activity1() {
        int[] i = new int[5];
        for (int j = 0; j < 5; j++) i[j] = NumberGetter.scanInt("Input " + (j + 1) + ": ");
        System.out.println(Arrays.toString(i));
        System.out.println("Max: " + Arrays.stream(i).max().getAsInt());
    }

    public static void activity2() {
        int i = NumberGetter.scanInt("Number of students: ");
        int[] scores = new int[i];
        for (int k = 0; k < i; k++) scores[k] = NumberGetter.scanInt("Student " + (k + 1) + " score: ");
    ...
    public static void activity5() {
        ...
        for (int i = 0; i < 12; i++) {
            ...
            months[0][i] = NumberGetter.scanDouble("Highest temperature: ");
            months[1][i] = NumberGetter.scanDouble("Lowest temperature: ");
        }
